...
(function () {
    ...
    angular.module('spring-security-csrf-token-interceptor', [])
        .config(function ($httpProvider) {
            ...
            csrfTokenData = { headerName: defaultCsrfTokenHeader, token: null },
            numRetries = 0,
            MAX_RETRIES = 5;
            ...
            $httpProvider.interceptors.push(function ($q, $injector) {
                return {
                    ...
                    response: function(response) {
                        // Edit by CN: Get token if it's already there
                        var csrfTokenHeader = response.headers(csrfTokenHeaderName);
                        csrfTokenData.headerName = csrfTokenHeader ? csrfTokenHeader : csrfTokenData.headerName;
                        var newToken = response.headers(csrfTokenData.headerName);
                        if (newToken) {
                            csrfTokenData.token = newToken;
                        }
